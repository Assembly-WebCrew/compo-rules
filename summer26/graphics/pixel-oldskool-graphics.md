# Pixel & Oldskool Graphics

Refer to the [General rules](../general.md) with the following additions:

A combined graphics competition for traditional pixeled graphics and legacy platform native formats. Submit your work as either a modern pixeled image or in a native legacy format. If your work is pixel art or made for a legacy platform, it belongs in this compo - even if its exact resolution or format is not mentioned in the examples below.
...
- The image resolution should be appropriate for pixel art. Typical examples: 320x200, 320x240, 320x256, 256x192, 160x200, or any other platform-native or similar low-res size. These are examples, not a complete list - if your resolution is not listed here, it is still welcome in this compo. Do not move your entry to Digital Graphics just because a resolution is missing from this list. 256 colors max.
- Submit as PNG. No animation.
- Submit a version of the final image with NO visible signature. Name this file with "_nosig" prefix (e.g. "cool_image.png" for the final image and "cool_image_nosig.png" for unsigned version of the same image). Entries without an unsigned version will be disqualified. You may optionally include a signed version too.
- You must submit at least three (3) intermediate stages showing the process of pixeling.
- Different steps of the process (e.g. outline, coloring, refinements) should be shown in different files from different points in time.
...
- Native format: you decide what counts as the native format for your platform and tools - we do not enforce a strict list. Typical examples: C64 multicolor, Amiga IFF/LBM, Atari ST .PI1/.NEO, ZX Spectrum .SCR. Always include a PNG or JPEG image of the work; judging and the big screen showing use that image, and we trust your choice of native format.
- Resolutions are not restricted to a list either: any resolution your platform natively supports (for example Amiga 320x256) is fine, even if it is not mentioned anywhere in these rules.
